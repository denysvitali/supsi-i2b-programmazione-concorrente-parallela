package it.denv.supsi.progconc.serie.serie6.es2.b;
...
class Helper implements Runnable {
	...
	public void run() {
		...
		while (true) {
			if (S6Esercizio2_HIO.sharedState != null)
				break;
		}

		int lastValue = S6Esercizio2_HIO.sharedState.getValue();
		...
		while (true) {
			final int curValue = S6Esercizio2_HIO.sharedState.getValue();
		...
		for (int i = 0; i < 5000; i++) {
			S6Esercizio2_HIO.sharedState.increment();
			if ((i % 100) == 0)
				try {
					Thread.sleep(1);
...
class Starter implements Runnable {
	...
	public void run() {
		...
		for (int i = 0; i < 5000; i++) {
			S6Esercizio2_HIO.sharedState = new ImmutableSharedState(S6Esercizio2_HIO.sharedState.getValue() + 1);
			if ((i % 100) == 0) {
				try {
					Thread.sleep(1);
				} catch (final InterruptedException e) { }
...
public class S6Esercizio2_HIO {
	...
	public static void main(final String[] args) {
		...
		System.out.println("Main: final value " + S6Esercizio2_HIO.sharedState.getValue());
	}
}
