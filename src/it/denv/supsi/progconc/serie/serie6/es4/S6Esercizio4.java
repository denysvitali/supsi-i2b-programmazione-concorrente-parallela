package it.denv.supsi.progconc.serie.serie6.es4;
...
class S6ServiziPubblici {
	...
	public S6ServiziPubblici(final int nrBagniUomini, final int nrBagniDonne) {
		this.bagniUomini = new Stack<Bagno>();
		for (int i = 0; i < nrBagniUomini; i++) {
			this.bagniUomini.push(new Bagno());
		}

		this.bagniDonne = new Stack<Bagno>();
		for (int i = 0; i < nrBagniDonne; i++) {
			this.bagniDonne.push(new Bagno());
		}
	}
	...
	public boolean occupaBagno(final boolean uomo) {
		...
		synchronized (this) {
			if (uomo) {
				...
				Bagno bagno = bagniDonne.peek();
				while (!bagniDonne.empty() && !bagno.provaOccupare()) {
					bagno = bagniDonne.pop();
				}
